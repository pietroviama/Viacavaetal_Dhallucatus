...
fitshapedistcoastmain <- procD.lm(gdfmainland$coords~Mainlanddata$distanceToCoast, effect.type = "F")
summary(fitshapedistcoastmain)

lmsizedistcoastmain <- lm(gdfmainland$Csize~Mainlanddata$distanceToCoast)
anova(lmsizedistcoastmain)
...
PCAminusislandplot <- plot(PCAminusisland$pc.scores[,1], PCAminusisland$pc.scores[,2], bg = gdfhallucatusminusisland$Sex, pch = 21, cex = 1.5, asp = T, xlab="Principal Component 1 (24.58%)", ylab="Principal Component 2 (11.58%)")
...
fit.size <- procD.lm(f1 = coords~Csize, data = gdfhallucatusminusisland, print.progress = FALSE)
plotAllometry(fit.size, size = gdfhallucatusminusisland$Csize, logsz = FALSE, method = "RegScore", pch = 19, col = as.numeric(gdfhallucatusminusisland$Population))
anova(fit.size)
...
islmainbplot <- ggplot(df.hallucatusislmain, 
  ...
  geom_boxplot() +
  theme_classic() +
  theme(legend.position = "none", axis.text = element_text(size = 16), axis.title.y = element_text(size=16))
