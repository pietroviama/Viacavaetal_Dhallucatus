...
#retrieve vegetation type from spatial data of the Atlas of Living Australia
Dhallgps <- data.frame(Dhallucatusdata$Latitude, Dhallucatusdata$Longitude)
layers <- c("cl620", "cl618", "el819", "el1077", "el674")
vegtype <- intersect_points(Dhallgps, layers)
vegtype$vegetationTypesPresent<-as.factor(vegtype$vegetationTypesPresent)
vegtype$landCover<-as.factor(vegtype$landCover)
vegtype$distanceToCoast[is.na(vegtype$distanceToCoast)]<-0
vegtype$elevation[is.na(vegtype$elevation)]<-0


vegtype2 <- data.frame(Dhallucatusdata$Museum_ID, Dhallucatusdata$Latitude, Dhallucatusdata$Longitude, Dhallucatusdata$Vegetation_type)

levels(vegtype2$Dhallucatusdata.Vegetation_type)[levels(vegtype2$Dhallucatusdata.Vegetation_type)=="Eucalypt open forests"]<-"Eucalyptus open forest"
levels(vegtype2$Dhallucatusdata.Vegetation_type)[levels(vegtype2$Dhallucatusdata.Vegetation_type)=="Eucalypt open woodlands"]<-"Eucalyptus open woodlands"
levels(vegtype2$Dhallucatusdata.Vegetation_type)[levels(vegtype2$Dhallucatusdata.Vegetation_type)=="Eucalypt woodlands"]<-"Eucalyptus woodlands"
levels(vegtype$vegetationTypesPresent)[levels(vegtype$vegetationTypesPresent)==""]<-"Unknown/no data"
levels(vegtype$landCover)[levels(vegtype$landCover)==""]<-"Unknown/no data"


vegtype2$Dhallucatusdata.Vegetation_type <- factor(vegtype2$Dhallucatusdata.Vegetation_type, levels=levels(vegtype$vegetationTypesPresent))

vegtype$vegetationTypesPresent==vegtype2$Dhallucatusdata.Vegetation_type

vegtypefill <- coalesce(vegtype2$Dhallucatusdata.Vegetation_type, vegtype$vegetationTypesPresent)

Dhallucatusdata$landCover <- vegtype$landCover
Dhallucatusdata$distanceToCoast <- vegtype$distanceToCoast
Dhallucatusdata$Vegetation_type <- vegtypefill
Dhallucatusdata$primprod <- vegtype$grossPrimaryProductivity20120313
Dhallucatusdata$elevation <- vegtype$elevation
...
gdfhallucatus <- geomorph.data.frame(gpahallucatus, Population = Dhallucatusdata$Population, 
                           ...
                           habitat = Dhallucatusdata$Forest_habitat, vegtype = Dhallucatusdata$Vegetation_type, landcover = Dhallucatusdata$landCover, distcoast = Dhallucatusdata$distanceToCoast, elevation = Dhallucatusdata$elevation, climate = Dhallucatusdata$Climate_classification,
...
gdfhallucatusminusisland <- geomorph.data.frame(gpadhallminusisland, Population = Dhalldataminusisland$Population, 
                           ...
                           ppworldclim = Dhallucatusworldclimminusisland$Prec, tempworldclim = Dhallucatusworldclimminusisland$Temp, Species = Dhalldataminusisland$Species, Dhalldataminusisland, landcover = Dhalldataminusisland$landCover, DistancetoCoast = Dhalldataminusisland$distanceToCoast, elevation = Dhalldataminusisland$elevation) 
```
...
gdfmainland <- geomorph.data.frame(gpamainland, Population = Mainlanddata$Population, 
                           ...
                           Species = Mainlanddata$Species, precworldclim = Dhallucatusworldclimmain$Prec, tempworldclim = Dhallucatusworldclimmain$Temp, landcover = Mainlanddata$landCover, DistancetoCoast = Mainlanddata$distanceToCoast, primprod = Mainlanddata$primprod, Mainlanddata$elevation) 

#ProcANOVA on shape with lon and lat factors
...
fitshapeppmain <- procD.lm(gdfmainland$coords~Dhallucatusworldclimmain$Prec)
...
fitshapedistcoastmain <- procD.lm(gdfmainland$coords~Mainlanddata$distanceToCoast)
summary(fitshapedistcoastmain)

lmsizedistcoastmain <- lm(gdfmainland$Csize~Mainlanddata$distanceToCoast)
summary(lmsizedistcoastmain)

fitshapeelevmain <- procD.lm(gdfmainland$coords~Mainlanddata$elevation)
summary(fitshapeelevmain)

lmsizeelevmain <- lm(gdfmainland$Csize~Mainlanddata$elevation)
summary(lmsizeelevmain)

fitshapeprimprodmain <- procD.lm(gdfmainland$coords~Mainlanddata$primprod)
summary(fitshapeprimprodmain)

lmsizeprimprodmain <- lm(gdfmainland$Csize~Mainlanddata$primprod)
summary(lmsizeprimprodmain)

fitshapevegtypemain <- procD.lm(gdfmainland$coords~Mainlanddata$Vegetation_type)
summary(fitshapevegtypemain)

lmsizevegtypemain <- lm(gdfmainland$Csize~Mainlanddata$Vegetation_type)
summary(lmsizevegtypemain)
...
disparitymainisl <- morphol.disparity(coords ~ 1, groups= ~ main_island, data = gdfhallucatus, iter=999)
summary(disparitymainisl)

disparitypops <- morphol.disparity(coords ~ 1, groups = ~ Population, data = gdfhallucatusminusisland, iter = 999)
summary(disparitypops)


fit3var <- lm(disparitypops$Procrustes.var~gdfhallucatusminusisland$Population)
summary(fit3var)
...
#Figure 1a: shape changes associated with PC1 

mshapePC1minmax<-coordinates.difference(PCAminusisland$pc.shapes$PC1min,PCAminusisland$pc.shapes$PC1max, type = "spherical")

procrustes.var.plot(PCAminusisland$pc.shapes$PC1max, PCAminusisland$pc.shapes$PC1min, col = list(grDevices::heat.colors, "black"), col.val = mshapePC1minmax[[1]][,"radius"], pt.size = 0.5)
...
#Figure 1b: shape changes associated with allometry 
...
procrustes.var.plot(predshapesize$predmin, predshapesize$predmax, col = list(grDevices::heat.colors, "black"), allomshapediff[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
#shape changes associated with precipitation
...
procrustes.var.plot(predshapepp$predmax, predshapepp$predmin, col = list(grDevices::heat.colors, "black"), ppshapediff[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(predshapetemp$predmax, predshapetemp$predmin, col = list(grDevices::heat.colors, "black"), tempshapediff[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
summary(fit1)
I acutalPW1<- pairwise(fit1, groups = Dhalldataminusisland$Population)
...
summary(PW2, test.type = "dist", confidence = 0.95, stat.table = FALSE)
...
gdfres <- geomorph.data.frame(gpares, Population = Dhalldataminusisland$Population, 
                           ...
                           Aridity = Dhalldataminusisland$Aridity_index, precipitation = Dhalldataminusisland$Precipitation, 
                           habitat = Dhalldataminusisland$Forest_habitat, vegtype = Dhalldataminusisland$Vegetation_type, climate = Dhalldataminusisland$Climate_classification,
                           Species = Dhalldataminusisland$Species,  landcover = Dhalldataminusisland$landCover, DistancetoCoast = Dhalldataminusisland$distanceToCoast)
...
summary(PW1resshape, test.type = "dist", confidence = 0.95, stat.table = FALSE)
...
procrustes.var.plot(mshapeGroote, mshapeKimberley, col = list(grDevices::heat.colors, "black"), col.val =  mshapegrootekimberley[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeGroote, mshapePilbara, col = list(grDevices::heat.colors, "black"), col.val =  mshapegrootePilbara[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeGroote, mshapeNT, col = list(grDevices::heat.colors, "black"), col.val =  mshapegrooteNT[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
rgl.snapshot("C:/Users/pietro/Desktop/Pietro/Projects/Viacavaetal_Dhallucatus/Analysis/Figures/Fig2_mshapegrooteNT", fmt = "png")

procrustes.var.plot(mshapeGroote, mshapeQLD, col = list(grDevices::heat.colors, "black"), col.val =  mshapegrooteQLD[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeQLD, mshapePilbara, col = list(grDevices::heat.colors, "black"), col.val =  mshapeQLDpilbara[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeQLD, mshapeNT, col = list(grDevices::heat.colors, "black"), col.val =  mshapeQLDNT[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
rgl.snapshot("C:/Users/pietro/Desktop/Pietro/Projects/Viacavaetal_Dhallucatus/Analysis/Figures/Fig2_mshapeQLDNT", fmt = "png")

procrustes.var.plot(mshapeQLD, mshapeKimberley, col = list(grDevices::heat.colors, "black"), col.val =  mshapeQLDKimberley[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeNT, mshapeKimberley, col = list(grDevices::heat.colors, "black"), col.val =  mshapeNTKimberley[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeNT, mshapePilbara, col = list(grDevices::heat.colors, "black"), col.val =  mshapeNTPilbara[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
procrustes.var.plot(mshapeKimberley, mshapePilbara, col = list(grDevices::heat.colors, "black"), col.val =  mshapeKimberleyPilbara[[1]][,"radius"], pt.size = 0.5)
rgl.viewpoint(180, -90, fov = 0, zoom = 0.5)
...
base <- get_map(location=sbbox, maptype="satellite", source = "google")
...
map2
north2 (map2, .15, .7, scale = 0.1, symbol = 1)
...
gdfsex <- geomorph.data.frame(gpasex, Population = Sexdata$Population, 
                           ...
                           habitat = Sexdata$Forest_habitat, vegtype = Sexdata$Vegetation_type, climate = Sexdata$Climate_classification, landcover = Sexdata$landCover, DistancetoCoast = Sexdata$distanceToCoast) 
...
procrustes.var.plot(mshapefemale, mshapemale, col = grDevices::heat.colors, col.val = mshapemalefemale[[1]][,"radius"], pt.size = 0.7)
...
CVAquollspops <- CVA(gdfmainland$coords, groups = gdfmainland$Population, weighting = TRUE, plot = TRUE, rounds = 1000)
plot(CVAquollspops$CVscores[,1:2], bg=gdfmainland$Population, pch=21, typ="p", asp = 1)
gpamainland$procD
Euclidist_quollspops <- CVAquollspops$Dist$GroupdistEuclid

Euclidist <- dist(gpamainland$procD, method = "euclidean")

colspop <- rainbow(length(levels(gdfmainland$Population)))
library(dendextend)
ClustLab <- paste(gdfmainland$Population)
plotColoredClusters(the.cluster, labs = ClustLab, cols = colspop)

population <- factor(gdfmainland$Population)
...
the.cluster <- as.dendrogram(hclust(Euclidist_quollspops, method = "average"))
dend2 <- color_labels(the.cluster, col = colspop, labels = labels(the.cluster)[gdfmainland$Population])
plot(dend2)
plot(the.cluster)
...
clusteronmeans<-function(x,group,method="euclidean",FUN=mean,nj=F,title=NULL,tip.color=1,cex=1){
 library(MASS)
 library(car)
 library(phangorn)
 x<-as.matrix(x)
...
popclustupgma <- clusteronmeans(gpamainland$procD, group = gdfmainland$Population)
...
islmainbplot <- ggplot(df.hallucatusislmain, 
  ...
  scale_fill_manual(values = c("gray84", "gray84")) +
  labs(x= NULL, y= "Centroid size",
       title= NULL) +
  geom_boxplot() +
  theme_classic() +
  theme(legend.position = "none", axis.text = element_text(size = 12), axis.title.y = element_text(size=12))
